			...
			<div>
				...
				<Label>
					...
					<Label.Detail>
						<Pretty
							className="value"
							value={runtime.version.specName}
						/>{" "}
						v
						<Pretty
							className="value"
							value={runtime.version.specVersion}
						/>{" "}
						(
						<Pretty
							className="value"
							value={runtime.version.implName}
						/>{" "}
						v
						<Pretty
							className="value"
							value={runtime.version.implVersion}
						/>
						)
					</Label.Detail>
				</Label>
				...
				<Label>
					...
					<Label.Detail>
						<Rspan className="value">
							{runtime.core.authorities.mapEach((a, i) => (
								<Identicon
									key={bytesToHex(a) + i}
									account={a}
									size={16}
				...
				<Label>
					...
					<Label.Detail>
						<Pretty
							className="value"
							value={runtime.balances.totalIssuance}
						/>
					</Label.Detail>
				</Label>
			</div>
			...
			<Segment style={{ margin: "1em" }} padded>
				<Header as="h2">
					...
					<Header.Content>
						Address Book
						<Header.Subheader>
							Inspect the status of any account and name it for
							later use
						</Header.Subheader>
					</Header.Content>
				</Header>
						...
						then={
							<div>
								<Label>
									...
									<Label.Detail>
										<Pretty
											value={runtime.balances.balance(
												this.lookup
											)}
										/>
									</Label.Detail>
								...
								<Label>
									...
									<Label.Detail>
										<Pretty
											value={runtime.system.accountNonce(
												this.lookup
											)}
										/>
									</Label.Detail>
								...
								<If
									condition={runtime.indices
										.tryIndex(this.lookup, null)
										.map(x => x !== null)}
									then={
										<Label>
											Short-form
		...
		return (
			<Segment style={{ margin: "1em" }} padded>
				...
				<div style={{ paddingBottom: "1em" }}>
					...
					<If
						condition={this.destination.ready()}
						then={
							<Label>
								Balance
								<Label.Detail>
				...
				<TransactButton
					...
					tx={{
						...
						call: calls.balances.transfer(
							runtime.indices.tryIndex(this.destination),
							this.amount
						),
						compact: false,
						longevity: true
...
class UpgradeSegment extends React.Component {
	constructor() {
		super();
		this.conditionBond = runtime.metadata.map(
			m =>
					...
					<Segment style={{ margin: "1em" }} padded>
						<Header as="h2">
							...
							<Header.Content>
								Runtime Upgrade
								<Header.Subheader>
									Upgrade the runtime using the UpgradeKey
									module
								</Header.Subheader>
							</Header.Content>
						</Header>
...
class PokeSegment extends React.Component {
	...
	render() {
		return (
			<If
				condition={runtime.metadata.map(
					m => m.modules && m.modules.some(o => o.name === "sudo")
				)}
				then={
					<Segment style={{ margin: "1em" }} padded>
						<Header as="h2">
							...
							<Header.Content>
								Poke
								<Header.Subheader>
									Set a particular key of storage to a
									particular value
								</Header.Subheader>
							</Header.Content>
						</Header>
						<div style={{ paddingBottom: "1em" }} />
						<InputBond
							bond={this.storageKey}
							placeholder="Storage key e.g. 0xf00baa"
						/>
						<InputBond
							bond={this.storageValue}
							placeholder="Storage value e.g. 0xf00baa"
						/>
						<TransactButton
							content="Poke"
							icon="warning"
							tx={{
								...
								call: calls.sudo
									? calls.sudo.sudo(
											calls.consensus.setStorage([
												[
													this.storageKey.map(
														hexToBytes
													),
													this.storageValue.map(
														hexToBytes
													)
												]
											])
